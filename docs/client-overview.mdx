...
`state` is the stage of the process life-cycle the process is currently at, and `status` is the execution outcome. `status` is `null` while the process has not yet completed.
...
Signals are a way of controlling processes at different stages of their life-cycle.
